...
import { Loader2, Crown, CheckCircle, Clock, CreditCard, Shield } from 'lucide-react';
              ...
              <div className="mb-6 space-y-4">
                <div className="bg-white/5 p-4 rounded-lg border border-white/10">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Badge variant="outline" className={`${subscription.plan !== 'free' ? 'bg-green-500/20 text-green-400' : 'bg-white/10'}`}>
                      {subscription.subscribed ? 'Active' : 'Free Tier'}
                    </Badge>
                  </div>
                  
                  {subscription.subscribed && subscription.current_period_end && (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2">
                    <div className="flex items-center gap-1">
                      ...
                      <span>{subscription.vehicles_limit < 0 ? 'Unlimited vehicles' : `${subscription.vehicles_limit} vehicles`}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <CheckCircle className="h-4 w-4 text-neon-blue" />
                      <span>{subscription.maintenance_limit < 0 ? 'Unlimited maintenance logs' : `${subscription.maintenance_limit} maintenance logs`}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <CheckCircle className={`h-4 w-4 ${subscription.ai_access ? 'text-neon-blue' : 'text-gray-500'}`} />
                      <span className={!subscription.ai_access ? 'text-gray-500' : ''}>
                        AI Assistant {subscription.ai_access ? 'Access' : 'No access'}
                      </span>
                    </div>
                    <div className="flex items-center gap-1">
                      <CheckCircle className={`h-4 w-4 ${subscription.ai_predictions ? 'text-neon-blue' : 'text-gray-500'}`} />
                      <span className={!subscription.ai_predictions ? 'text-gray-500' : ''}>
                        AI Predictions {subscription.ai_predictions ? 'Enabled' : 'Disabled'}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {plans.map((plan) => (
                  <div 
                    ...
                    } ${subscription?.plan === plan.id ? 'ring-2 ring-neon-blue' : ''}`}
                  >
                    {subscription?.plan === plan.id && (
                      <Badge className="bg-neon-blue text-black mb-2">Current Plan</Badge>
                    )}
                    ...
                    {subscription?.plan !== plan.id && plan.id !== 'free' && (
                      <Button 
                        className={`w-full ${plan.highlighted ? 'bg-neon-blue hover:bg-neon-blue/90 text-black' : ''}`}
                    ...
                    {subscription?.plan !== plan.id && plan.id === 'free' && subscription?.subscribed && (
                      <Button 
                        variant="outline"
                        className="w-full border-white/10"
                        disabled={portalLoading}
