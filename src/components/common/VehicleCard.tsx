...
const VehicleCard = ({
  ...
  const displayImg = useMemo(() => (imgErr ? null : imgSrc), [imgErr, imgSrc]);
      ...
      <div className='relative h-48 overflow-hidden'>
        {displayImg ? (
          <img
            ...
            onError={() => {
              if (DEBUG_MEDIA) {
                console.warn('[media] <img> onError', { id, src: displayImg });
              }
              setImgErr(true);
            }}
          />
        ...
        {DEBUG_MEDIA && (
          <div className='absolute left-2 bottom-2 text-[10px] max-w-[90%] break-all opacity-70 bg-black/50 px-1.5 py-0.5 rounded'>
            src: {displayImg ?? '(none)'}
          </div>
        )}
        ...
        <div className='absolute top-3 right-3 z-10'>
          <DropdownMenu>
            ...
            <DropdownMenuContent
              align='end'
              className='bg-dark-card border-white/10'
            >
              <DropdownMenuItem
      ...
      <div className='p-5 space-y-4'>
        ...
        <div className='border-t border-white/5 pt-4 flex justify-between items-center'>
          <div className='flex items-center gap-2 text-sm'>
            ...
            <span>
              Next service:{' '}
              <span className='text-foreground/70'>{nextService}</span>
            </span>
          </div>
          <Button
            variant='outline'
