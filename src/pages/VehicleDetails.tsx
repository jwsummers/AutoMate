...
import { ChevronLeft, Car, Wrench, Calendar, AlertTriangle, ArrowRight, Edit, Clock } from 'lucide-react';
...
const VehicleDetails = () => {
  ...
  const { maintenanceRecords, loading: maintenanceLoading, markMaintenanceAsCompleted } = useMaintenance();
  ...
  useEffect(() => {
    ...
    const records = maintenanceRecords.filter(record => record.vehicle_id === id);
        ...
        <main className="flex-1 pt-28 pb-16">
          <div className="container mx-auto px-4">
            <div className="text-center py-12">
              ...
              <p className="text-foreground/70 mb-6">The vehicle you're looking for doesn't exist or you don't have permission to view it.</p>
              <Button asChild>
  ...
  const upcomingCount = vehicleMaintenance.filter(m => m.status === 'upcoming').length;
  const overdueCount = vehicleMaintenance.filter(m => m.status === 'overdue').length;
  const completedCount = vehicleMaintenance.filter(m => m.status === 'completed').length;
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-8">
              <div>
                ...
                <p className="text-foreground/70 mt-1">{vehicle.mileage ? `${vehicle.mileage.toLocaleString()} miles` : 'Mileage not specified'}</p>
              </div>
              <Button asChild className="bg-neon-blue hover:bg-neon-blue/90 text-black">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <Card className="bg-dark-card border-white/10">
                <CardContent className="pt-6">
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="text-foreground/70 text-sm mb-1">Health Score</p>
                      <h3 className="text-3xl font-bold">{healthScore}%</h3>
                    </div>
                    <div className={`p-2 rounded-lg ${healthScore >= 80 ? 'bg-green-500/10' : healthScore >= 50 ? 'bg-yellow-500/10' : 'bg-red-500/10'}`}>
                      <Car className={`h-5 w-5 ${healthScore >= 80 ? 'text-green-500' : healthScore >= 50 ? 'text-yellow-500' : 'text-red-500'}`} />
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="bg-dark-card border-white/10">
                <CardContent className="pt-6">
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="text-foreground/70 text-sm mb-1">Overdue Services</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <span className="font-medium">{vehicle.mileage ? `${vehicle.mileage.toLocaleString()} miles` : 'Not specified'}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex items-center justify-between">
                            <span className="text-foreground/70">License Plate:</span>
                            <span className="font-medium">{vehicle.license_plate || 'Not specified'}</span>
                          </div>
                          <div className="flex items-center justify-between">
                    ...
                    <Card className="bg-dark-card border-white/10">
                      <CardContent className="p-6 text-center">
                        ...
                        <p className="text-foreground/70 mb-4">
                          You haven't added any maintenance records for this vehicle yet.
                        </p>
                        <Button asChild>
                          <Link to={`/dashboard?addMaintenance=${vehicle.id}`}>Add First Maintenance Record</Link>
                        </Button>
                      </CardContent>
                    </Card>
                  ) : (
              ...
              <div>
                ...
                <Card className="bg-dark-card border-white/10 overflow-hidden mb-8">
                  {vehicle.image ? (
                    <img src={vehicle.image} alt={`${vehicle.year} ${vehicle.make} ${vehicle.model}`} className="w-full aspect-video object-cover" />
                  ) : (
